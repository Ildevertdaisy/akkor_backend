...
async def register(request: Request,  newUser: UserBase = Body(...)) -> UserBase:
    ...
    # newUser = UserBase(email=email, password=password, username=pseudo, role="CUSTOMER")
    # print(newUser)
...
async def login(request: Request, loginUser: LoginBase = Body(...)) -> str:
    loginUser = jsonable_encoder(loginUser)
    print(loginUser)
    ...
    response = JSONResponse(status_code=status.HTTP_201_CREATED, content={"token": token})
